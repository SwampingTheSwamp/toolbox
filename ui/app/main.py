...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.stack.addWidget(self.data_screen)  # 1
        self.stack.addWidget(self.ml_screen)    # 2
    ...
    def create_ml_screen(self):
        ...
        self.load_ml_btn = QPushButton("Carregar dataset para ML")
        self.load_ml_btn.clicked.connect(self.prepare_ml_dataset)
        ...
        self.ml_result = QLabel("Nenhum modelo treinado.")

        layout.addLayout(top_layout)
        layout.addWidget(title)
        layout.addWidget(self.load_ml_btn)
        layout.addWidget(QLabel("Coluna alvo:"))
        layout.addWidget(self.target_combo)
        layout.addWidget(train_btn)
        layout.addWidget(self.ml_result)
        layout.addStretch()
    ...
    def prepare_ml_dataset(self):
        if self.current_df is None:
            self.ml_result.setText("Carregue um CSV no módulo de dados primeiro.")
            return

        self.target_combo.clear()
        self.target_combo.addItems(
            self.current_df.select_dtypes(include="number").columns
        )

        self.target_combo.setEnabled(True)
        self.ml_result.setText("Dataset pronto para ML.")
    ...
    def train_model(self):
        ...
        model, mse = train_linear_regression(self.current_df, target)

        if model is None:
            self.ml_result.setText("Erro: não há colunas numéricas suficientes.")
            return
